...
### Mevcut Sürüm Yetenekleri

- **PID Controller**: Oransal (P), İntegral (I) ve Türev (D) bileşenleri aktiftir.
- **Feed Forward**: İstenilen hedefe daha hızlı oturmak için ileri besleme desteği kodlanmıştır.
- **Anti-Windup**: İntegral taşırmasına (Integral Windup) karşı tavan limitler belirlenmiştir.
- **Output Saturation**: Motorlara yollanacak maksimum `effort` sınırlandırılmıştır zira donanım yanmasını engellemek ana görevdir.
