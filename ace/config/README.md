...
## 5. Seri Numara Yaşam Döngüsü

`serial_number`, cihazın **donanım kimliği** olarak `PersistentConfig` içinde tutulur.

| Olay | Seri Numaraya Etkisi |
|---|---|
| İlk açılış (NVS boş) | `device_config.hpp::kDefaultSerialNumber` (örn: `SN-0000`) yüklenir |
| `SET_SERIAL <no>` | NVS'e yazılır, kalıcı |
| `FACTORY_RESET` | **Korunur** — yazılım ayarları sıfırlanır ama seri numara silinmez |
| Firmware güncellemesi (sürüm uyuşmazlığı) | **Korunur** — sadece diğer `PersistentConfig` alanları sıfırlanır |

---

## 6. `GET_INFO` Yanıtı

`GET_INFO` komutu gönderildiğinde sistem hem `device_config.hpp` hem de `PersistentConfig`'ten bilgileri toplayarak şu formatla yanıt verir:

```
INFO DEVICE_NAME=ACE-PT-01 DEVICE_ID=0xACE1 FW_VERSION=1.0.0
INFO SERIAL=SN-ACE-01-A BUILD_TARGET=ESP32-S3
INFO PAN_LIMIT_MIN=-180.0 PAN_LIMIT_MAX=180.0
INFO TILT_LIMIT_MIN=-90.0 TILT_LIMIT_MAX=90.0
```
